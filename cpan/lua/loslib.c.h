...
static int os_execute (lua_State *L) {
  ...
  if (cmd != NULL)
    ...
    marpa_lua_pushboolean(L, stat);  /* true if there is a shell */
    return 1;
  }
}
...
static int getfield (lua_State *L, const char *key, int d, int delta) {
  int isnum;
  int t = marpa_lua_getfield(L, -1, key);
  lua_Integer res = marpa_lua_tointegerx(L, -1, &isnum);
  if (!isnum) {  /* field is not a number? */
    if (t != LUA_TNIL)  /* some other value? */
      return marpa_luaL_error(L, "field '%s' not an integer", key);
    else if (d < 0)  /* absent field; no default? */
      return marpa_luaL_error(L, "field '%s' missing in date table", key);
    res = d;
  }
  else {
...
static const char *checkoption (lua_State *L, const char *conv, char *buff) {
  ...
  marpa_luaL_argerror(L, 1,
    marpa_lua_pushfstring(L, "invalid conversion specifier '%%%s'", conv));
...
static int os_date (lua_State *L) {
  ...
  if (stm == NULL)  /* invalid date? */
    marpa_luaL_error(L, "time result cannot be represented in this installation");
  if (strcmp(s, "*t") == 0) {
...
static int os_difftime (lua_State *L) {
  ...
  marpa_lua_pushnumber(L, (lua_Number)difftime(t1, t2));
  return 1;
}
