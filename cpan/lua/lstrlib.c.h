...
static int str_reverse (lua_State *L) {
  ...
  luaL_Buffer b;
  const char *s = marpa_luaL_checklstring(L, 1, &l);
  char *p = marpa_luaL_buffinitsize(L, &b, l);
  for (i = 0; i < l; i++)
    p[i] = s[l - i - 1];
...
static int str_rep (lua_State *L) {
  ...
  if (n <= 0) marpa_lua_pushliteral(L, "");
    ...
    luaL_Buffer b;
    char *p = marpa_luaL_buffinitsize(L, &b, totallen);
...
static int str_dump (lua_State *L) {
  luaL_Buffer b;
  int strip = marpa_lua_toboolean(L, 2);
  marpa_luaL_checktype(L, 1, LUA_TFUNCTION);
  marpa_lua_settop(L, 1);
  marpa_luaL_buffinit(L,&b);
  if (marpa_lua_dump(L, writer, &b, strip) != 0)
    return marpa_luaL_error(L, "unable to dump given function");
...
static int check_capture (MatchState *ms, int l) {
  ...
  if (l < 0 || l >= ms->level || ms->capture[l].len == CAP_UNFINISHED)
    return marpa_luaL_error(ms->L, "invalid capture index %%%d", l + 1);
  return l;
}
...
static const char *classend (MatchState *ms, const char *p) {
  switch (*p++) {
    case L_ESC: {
      if (p == ms->p_end)
        marpa_luaL_error(ms->L, "malformed pattern (ends with '%%')");
      return p+1;
    }
    case '[': {
      if (*p == '^') p++;
      do {  /* look for a ']' */
        if (p == ms->p_end)
          marpa_luaL_error(ms->L, "malformed pattern (missing ']')");
...
static const char *matchbalance (MatchState *ms, const char *s,
                                   const char *p) {
  if (p >= ms->p_end - 1)
    marpa_luaL_error(ms->L, "malformed pattern (missing arguments to '%%b')");
  if (*s != *p) return NULL;
  else {
    int b = *p;
    int e = *(p+1);
...
static const char *match (MatchState *ms, const char *s, const char *p) {
  if (ms->matchdepth-- == 0)
    marpa_luaL_error(ms->L, "pattern too complex");
      ...
      case L_ESC: {  /* escaped sequences not in the format class[*+?-]? */
        switch (*(p + 1)) {
          ...
          case 'f': {  /* frontier? */
            ...
            p += 2;
            if (*p != '[')
              marpa_luaL_error(ms->L, "missing '[' after '%%f' in pattern");
...
static void push_onecapture (MatchState *ms, int i, const char *s,
  ...
  if (i >= ms->level) {
    if (i == 0)  /* ms->level == 0, too */
      ...
      marpa_luaL_error(ms->L, "invalid capture index %%%d", i + 1);
  }
  else {
...
static int str_find_aux (lua_State *L, int find) {
  size_t ls, lp;
  const char *s = marpa_luaL_checklstring(L, 1, &ls);
  const char *p = marpa_luaL_checklstring(L, 2, &lp);
  lua_Integer init = posrelat(marpa_luaL_optinteger(L, 3, 1), ls);
  if (init < 1) init = 1;
  else if (init > (lua_Integer)ls + 1) {  /* start after string's end? */
    marpa_lua_pushnil(L);  /* cannot find anything */
    return 1;
  }
  /* explicit request or no special characters? */
  if (find && (marpa_lua_toboolean(L, 4) || nospecials(p, lp))) {
    /* do a plain search */
...
static int gmatch (lua_State *L) {
  ...
  const char *p = marpa_luaL_checklstring(L, 2, &lp);
  GMatchState *gm;
  marpa_lua_settop(L, 2);  /* keep them on closure to avoid being collected */
  gm = (GMatchState *)marpa_lua_newuserdata(L, sizeof(GMatchState));
...
static int marpa_lua_number2strx (lua_State *L, char *buff, int sz,
  ...
  if (fmt[SIZELENMOD] == 'A') {
    ...
    marpa_luaL_error(L, "modifiers for format '%%a'/'%%A' not implemented");
  return n;
}
...
static int str_format (lua_State *L) {
  ...
  while (strfrmt < strfrmt_end) {
    if (*strfrmt != L_ESC)
      ...
      switch (*strfrmt++) {
        case 'c': {
          nb = l_sprintf(buff, MAX_ITEM, form, (int)marpa_luaL_checkinteger(L, arg));
          break;
        }
        case 'd': case 'i':
        ...
        case 's': {
          size_t l;
          const char *s = marpa_luaL_tolstring(L, arg, &l);
        ...
        default: {  /* also treat cases 'pnLlh' */
          return marpa_luaL_error(L, "invalid option '%%%c' to 'format'",
                               *(strfrmt - 1));
        }
      }
...
static int getnumlimit (Header *h, const char **fmt, int df) {
  ...
  if (sz > MAXINTSIZE || sz <= 0)
    marpa_luaL_error(h->L, "integral size (%d) out of limits [1,%d]",
                     sz, MAXINTSIZE);
  return sz;
}
  ...
  *size = 0;  /* default */
  switch (opt) {
    ...
    case 'c':
      *size = getnum(fmt, -1);
      if (*size == -1)
        marpa_luaL_error(h->L, "missing size for format option 'c'");
      return Kchar;
    case 'z': return Kzstr;
    ...
    default: marpa_luaL_error(h->L, "invalid format option '%c'", opt);
  }
  return Knop;
}
...
static KOption getdetails (Header *h, size_t totalsize,
  ...
  if (opt == Kpaddalign) {  /* 'X' gets alignment from following option */
    if (**fmt == '\0' || getoption(h, fmt, &align) == Kchar || align == 0)
      marpa_luaL_argerror(h->L, 1, "invalid next option for option 'X'");
  }
  if (align <= 1 || opt == Kchar)  /* need no alignment? */
    *ntoalign = 0;
    ...
    if ((align & (align - 1)) != 0)  /* is 'align' not a power of 2? */
      marpa_luaL_argerror(h->L, 1, "format asks for alignment not power of 2");
...
static void packint (luaL_Buffer *b, lua_Unsigned n,
  ...
  char *buff = marpa_luaL_prepbuffsize(b, size);
  int i;
...
static int str_pack (lua_State *L) {
  ...
  initheader(L, &h);
  marpa_lua_pushnil(L);  /* mark to separate arguments from string buffer */
  marpa_luaL_buffinit(L, &b);
  while (*fmt != '\0') {
    int size, ntoalign;
    ...
    switch (opt) {
      ...
      case Kfloat: {  /* floating-point options */
        volatile Ftypes u;
        char *buff = marpa_luaL_prepbuffsize(&b, size);
        lua_Number n = marpa_luaL_checknumber(L, arg);  /* get argument */
      ...
      case Kchar: {  /* fixed-size string */
        ...
        if ((size_t)size <= len)  /* string larger than (or equal to) needed? */
          marpa_luaL_addlstring(&b, s, size);  /* truncate string to asked size */
...
static int str_packsize (lua_State *L) {
  ...
  while (*fmt != '\0') {
    ...
    switch (opt) {
      case Kstring:  /* strings with length count */
      case Kzstr:    /* zero-terminated string */
        marpa_luaL_argerror(L, 1, "variable-length format");
  ...
  if (size < SZINT) {  /* real size smaller than lua_Integer? */
    ...
    for (i = limit; i < size; i++) {
      if ((unsigned char)str[islittle ? i : size - 1 - i] != mask)
        marpa_luaL_error(L, "%d-byte integer does not fit into Lua Integer", size);
    }
  }
...
static int str_unpack (lua_State *L) {
  ...
  while (*fmt != '\0') {
    ...
    if ((size_t)ntoalign + size > ~pos || pos + ntoalign + size > ld)
      marpa_luaL_argerror(L, 2, "data string too short");
    pos += ntoalign;  /* skip alignment */
    /* stack space for item + next position */
    marpa_luaL_checkstack(L, 2, "too many results");
    n++;
    switch (opt) {
      case Kint:
      case Kuint: {
  ...
  marpa_lua_pushinteger(L, pos + 1);  /* next position */
  return n + 1;
}
