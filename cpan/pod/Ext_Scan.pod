...
unlike internal scanning in which Marpa::R3 tokenizes a string for the app,
...
This section describes the usual behaviors for the
external scanning of a single token.
Some methods, for example C<lexeme_read_block()>,
behave differently
in a few specifics.
Whenever a method does not follow the rules described
in this section,
the difference will be specificly noted in the
...
For every external scan of a token which is not
a hard failure, there is a block span,
which is specified as described for each method.
For the purposes of this section, let that
block span be C<< <$block_id, $offset, $length> >>.
In that case, we'll define B<eolexeme>, the "end of lexeme"
...
If external scanning succeeds and no event occurs,
the current block is set to C<$block_id>.
and the current offset is set to the eolexeme.
The current eoread of the current block will
not be changed.

=item *

If external scanning succeeds and an event occurs,
the current block is set to C<$block_id>.
and the current offset is set to the event location.
The current eoread of the current block will
not be changed.
...
As a special case of the immediately preceding case,
if external scanning succeeds and a pre-lexeme event occurs,
the current block is set to C<$block_id>.
...
external scanning results in a soft failure.
In this case the current block data
remains unchanged.

=item *

Any failure in external scanning,
other than token rejection,
is a hard failure.
In this case no guarantee is made about the current block data.
...
If external scanning succeeds,
and a pre-lexeme event does not occur,
Marpa::R3 advances
the current G1 location by one.
The token read will start at the previous G1 location
...
If external scanning has a soft failure,
the current G1 location will remain where it was
before external scanning.

If external scanning has a hard failure,
no guarantee is made about the current G1 location.
Marpa::R3 will attempt to leave it valid and unchanged.

=head2 Event handlers

On success, the event handlers will see
a G1 location and an event location as described
above.
Event handlers are not invoked on failure.
...
If the symbol is rejected,
C<lexeme_read_block()> has a soft failure.
On soft failure, no token is read,
and the current block data
and the G1 location are unchanged.

If 
C<lexeme_read_block()> succeeds and no event occurs,
...
If 
C<lexeme_read_block()> succeeds and a parse event triggers,
the current block will be set to C<$block_id>,
and the current offset of the current block
