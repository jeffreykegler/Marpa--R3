...
An method is called an external scanning completion method,
or just a B<completion method>,
if it might complete external scanning at a G1 location.
...
Some completion methods, for example C<lexeme_read_string()>
and C<lexeme_complete()>,
behave differently
in a few specifics.
Whenever an external scanning completion method
does not follow the rules described
in this section,
...
or else that completion results in a hard failure.
