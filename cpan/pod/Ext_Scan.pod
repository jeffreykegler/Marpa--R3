...
In that case, we'll define the B<eolexeme>, or "end of lexeme",
...
The current offset is set to the eolexeme.
...
may occur during external scanning.
...
C<lexeme_read_block()> performs external scanning
L<as described above|"External scanning in detail">.
...
C<lexeme_read_literal()> performs external scanning
as described above,
in the secton on L</"External scanning in detail">.
...
(A pedantic exception to this is the case of a hard failure:
hard failures leave the current block data undefined.)
