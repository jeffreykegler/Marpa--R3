...
=head1 THIS INTERFACE IS BEING REMOVED

THIS INTERFACE IS UNDERGOING REMOVAL.
This document will eventually be removed.
The interface itself is broken,
will be totally unusable for those not on Marpa::R3's development
team.

The rest of this document is the pre-breakage original.
It is being retained for the convenience
of Marpa::R3's development team, which
...
are
L<< C<< Marpa::R3::Thin::G->new() >>|C<< Marpa::R3::Thin::G->new() >> >>,
L<< C<throw_set()>|C<< $g->throw_set() >> >>
and L<< C<ruby_slippers_set()>|C<< $r->ruby_slippers_set() >> >>.
...
The hash argument is required, though it may be empty.
At present, none of these named arguments are documented.
...
see L<< C<ruby_slippers_set()>|C<< $r->ruby_slippers_set() >> >>.
