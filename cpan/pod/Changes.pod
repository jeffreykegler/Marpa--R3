...
Marpa::R3, in the future, will allow the 
C<< slr->read() >> method to be called multiple
times, each time with a new string.
These strings will be called "blocks".

In preparation for this, the
C< $slr->input_length() >> call allows a new,
