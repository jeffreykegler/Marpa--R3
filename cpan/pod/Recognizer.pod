...
If C<block_id> is missing or undefined, it defaults to the current block.
If C<offset> is missing or undefined,
it defaults to the current offset of the specified block.
If C<length> is undefined, it is set so that the span is to the eoread of the
specified block.
If C<length> is defined and positive,
but would produce a span that goes beyond eoread,
it is truncated.
...
is reserved for unthrown failures.
...
B<Return values>:
If C<value()> successful produced a parse value,
...
C<last_completed()> returns the empty array.
Failure is always thrown.
...
C<line_column()> never changes the current block setting
of the recognizer,
the current offset of the current block
or
eoread of the current block.
...
it is a fatal error.
In what follows,
C<eoread> is the eoread of block C<block-id>.
...
It is a fatal error if C<block-id> is undefined
and there is not current block.

=item *
C<offset> is C<offset-arg>,
converted to a non-negative offset,
if 
C<offset-arg> is defined.
Otherwise C<offset> is the current offset
of C<block_id>.
It is a fatal error is C<offset-arg> cannot
be converted to a non-negative offset.

=item *
If C<length-arg> is not defined,
then C<length> is
C<eoread> - C<offset>.
If C<length-arg> is defined,
C<length> is C<length-arg>,
...
It is a fatal error is C<length-arg> cannot
