...
=item * For some string and grammar,
...
=item * A non-nulled symbol instance, non-nulled instance,
...
Intuitively, if we know the input as far as location I<L>
and a symbol I<< <S> >> is in some parse that starts at I<L>,
we say that symbol I<< <S> >> is B<acceptable> at
a location I<L>.
Where location I<L> is understood, we just say
that symbol I<< <S> >> is B<acceptable>.
Restating this more formally,
...
Intuitively, once we know the parse as far
as location I<L>,
if a symbol I<< <S> >> ends at I<L>,
we say that symbol I<< <S> >> is 
is B<recognized> at
a location I<L>.
Restating this more formally,
if an instance of symbol I<< <S> >>
...
A parse event is either a B<global parse status event>
or a B<parse instance event>.
B<Global parse status events>
are those which are deal with the status of the
parse as a whole.
B<Parse instance events> are those which report
deal with the status of a symbol instance.

There are two types
of global parse status event:
...
A parse instance event is either
a lexeme event or
a non-lexeme event.
...
will state whether it is a global parse status event,
a lexeme event
...
If an event pauses the recognizer,
it causes the
...
Some event-triggering methods return,
in any case,
after processing at a single location is finished,
and for those methods, it makes no difference
whether an event handler pauses the recognizer.
...
In the case of a pre-lexeme event, the recognizer
will only parse as far as the event location,
which will be earlier than the trigger location.
This means that pre-lexeme events act as a form
of look-ahead.
...
it is not scanned
by a call of the
L<C<< $slr->lexeme_complete() >> method|Marpa::R3::Scanless::R/"lexeme_complete()">
or of the
L<C<< $slr->lexeme_read() >> method|Marpa::R3::Scanless::R/"lexeme_read()">
method.
...
As one example,
the
L<C<lexeme_complete()>|Marpa::R3::Scanless::R/"lexeme_complete()"> and
L<C<lexeme_read()>|Marpa::R3::Scanless::R/"lexeme_read()">
...
L<C<< $slr->lexeme_complete() >>|Marpa::R3::Scanless::R/"lexeme_complete()"> and
L<C<< $slr->lexeme_read() >>|Marpa::R3::Scanless::R/"lexeme_read()"> methods.
