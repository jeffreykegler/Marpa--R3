...
=item * A string is B<actual to L> if we
...
the event location is before the
the trigger location.
...
the recognizer,
the app can enforce a form of look-ahead.
...
The event handlers are specified using the
L<C<event_handlers> name
argument|Marpa::R3::Scanless::R/"event_handlers">
of the 
L<the recognizer's C<new()>
constructor|Marpa::R3::Scanless::R/Constructor>.

A variable number of arguments is
passed to the event handler.
The first argument is always the the recognizer.
The subsequent arguments are the event data.
The first argument of the event data is
always the event name.
Whether there is more than one argument in the
event data,
and what the second and later arguments are,
depends on the type of event.
The details of the event data arguments are
described, for each type of event,
L<below|/"Types of parse event">.
...
at a parse location return
C<'pause'>,
the recognizer pauses at that
parse location.
If none of event handlers triggered
at a parse location return
C<'pause'>,
the recognizer only pauses at that
parse location if the method
that triggered the event would ordinarily
pause at that parse location.

Readers might find it intuitive to
think of a return of C<'ok'>
by a parse handler
as a command to
...
a C<'pause'> commands overrides
any number of C<'ok'> commands.

Another way to understand the return values
from event handlers,
is to think of
a return value of C<'ok'> from an event
handler as a no-op.
