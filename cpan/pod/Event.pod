...
which event an event can be created is the prediction of a symbol
in the parse.
...
Some recognizer methods
may not called inside a event handler for
the same recognizer.
The restricted methods include all those
recognizer methods which may themselves
trigger parse events are not allowed.
Restricted methods include
L<C<read()>|Marpa::R3::Scanless::R/"read()">,
L<C<resume()>|Marpa::R3::Scanless::R/"resume()">,
L<C<lexeme_read()>|Marpa::R3::Scanless::R/"lexeme_read()"> and
L<C<lexeme_complete()>|Marpa::R3::Scanless::R/"lexeme_complete()">.
It is allowed to call a method of one recognizer inside
an event handler belonging to a different recognizer.
