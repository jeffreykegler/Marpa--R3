...
The possibilities might overlap,
and optimizing one decision might make
the parse as a whole suboptimal.

We will say that the "longest highest" parse is the
parse with the fewest "items".
...
We define the "size" of a parse set,
C<Size(ps)>,
to be the minimum "size"
of any parse in C<p-set>.
If C<Size(p-hi) = Size(ps)>,
...
we see that it is a list of items,
...
B<(2)>:
Analyzing the grammar C<G>,
we can see that two C<VAR>s never occur
together in the same item.
This means that wherever we do find two
C<VAR>s in a row, it must mark a boundary
between items must occur between
the two successive C<VAR>s.
...
Note that, while all four strings in the display above
