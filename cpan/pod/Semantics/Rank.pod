...
The L<less formal justification of the grammar
in the
synopsis|"Longest highest, version 1">
relied on an unstated assumption:
that if we optimized the metric used in ranking
at each choicepoint,
this would optimize it for the whole document.
This is true for the synopsis grammar,
but it is not true in general.

For this section,
we consider a
tomonoid C<(M, +, >=)>
where M is a set of metrics,
C<+> is the monoid operation,
and C<< <= >
is an ordering relation over M.
A "tomonoid" is also called
a totally ordered monoid.
(The best known tomonoid is C<(N0, +, >=)>
where C<N0> is the set of natural numbers plus
zero, and C<+> and C<< <= > are the operations
of addition and "less than or equal" as ordinarily
defined.)
...
a choicepoint, or
a parse set.
We require the following of a metric:

B<Met-parse-set>:
The metric of a parse set is the metric of its
top choicepoint.

B<Met-choicepoint>:
The metric of a choicepoint is the maximum
over the metrics of the parse choices
that are the elements of that choicepoint.

B<Met-parse-choice>:
The metric of a parse choice is
C<Met(pred)+Met(cause)>.
Here C<+> is the tomonoid operation,
which is not necessarily ordinary addition;
C<pred> is the predessor choicepoint;
and C<cause> is the cause,
which may either be a choicepoint or a symbol.

B<Met-token>:
The metric of a token symbol is a constant.

Where C<m> is an element of C<M>,
C<R(m)> is the "rank" of C<m>.
Rank is an integer.
We assume that, within a choicepoint,
C<R> preserves the total order,
or more precisely, that

If C<m1> and C<m2> are both elements of the
tomonoid C<M>,
and C<m1 <= m2>,
then C<R(m1) <= R(m2)>.
...
are removed from it.
Where C<pset> is a parse set,
we say that C<Met(parse)> is the metric
of the parse.
C<Met(P-ranked)> is the metric of the original
ranked parse set,
...
Let C<cp> be a choicepoint in C<ps1>
which has at least two parse choices
...
To show the step, we assume that a C<ps1>
is a parse set,
and that it contains a C<cp> with
at two least two parses choices
with different metrics.
Using ranking, 
...
remains unchanged.
All other choicepoints remain exactly as
they were in C<ps1>,
so that,
by C<Met-choicepoint>,
their metrics also remain unchanged.
...
Marpa grammars have no cycles, so that a
induction by depth,
starting from the deepest leaf node
and proceeding to the
parse set's top choicepoint
will show
that no choicepoint changes its metric.
There the top choicepoint of C<ps2>
does not change its
metric from the metric
of top choicepoint of C<ps1>,
and 
C<Met(ps2) = Met(ps1)>.
This shows the step of the induction.

To show the induction terminates,
we note that the parse set contains a finite
number of choicepoints,
and that each induction step reduces one of these.
