...
Both Marpa's DSL and PEG allow the implementor to specify
...
fakes the rest.
(In math-y terms, the grammars PEG parses accurately
are the LL(1) grammars.)

This means that in Marpa,
the implementor can first write an unranked form of
their grammar in BNF,
an a "safety net".
The BNF grammar will specify
a superset of what will
be returned after ranking.
Ranking or no ranking,
no parse trees will ever be returned unless that BNF
accurately describes them.

With the "safety net" grammar established,
the implementor of a ranked grammar in Marpa then
uses ranks to prune,
eliminating parse trees until the only the ones desired
are returned.
Whether in Marpa or in PEG,
ranking can tricky.
For the grammar in the synopsis,
L<below we give a
proof|"Proof of correctness">
that the parses returned
are actually, and only, those desired.

Even if the implementor does not resort to a proof,
the Marpa BNF provides the "safety net".
...
it will usually be the best and safest choice.
