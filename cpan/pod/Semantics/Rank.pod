...
Rule ranking is given its own document,
because it merits careful definition,
so that multiple examples can be given.
...
Put more directly, the typical PEG implementor does not know what
language they implemented.
PEG implementations often do not parse strings allowed
by the BNF grammar.
This is usually what the PEG implementor tests for,
but it is not the only problem,
and may not be the worst problem.

PEG implementations also often parse strings not allowed by
the BNF grammar.
This is not a minor issue:
overliberal parsers can and have cause security issues.

Overliberal parser is sometimes described as a PEG capability:
PEG can parse non-context-free languages.
But this is a little like saying that a sedan with bad steering
and a tricky suspension has off-road capabilities.

PEG's non-context-free parsing would be a feature if it was
predictable and describable.
Regular expressions and context-free BNF specifications are
popular because they allow the programmer to examine them
and predict the behaviors of the parsers which implement them.
A "capability" which cannot be controlled or described by the implementor
is in fact a danger.

With Marpa, no matter how you assign rule ranks,
and no matter which ranking method you use,
you will never get a parse which is not compatible with the
Marpa grammar interpreted as BNF.
Rule ranking reduces the parses returned to a subset
of those allowed by the BNF grammar.
This gives the implementor which decides to use rule ranking
a vital "safety net".

Additionally, since Marpa parses all context-free
grammars, it allows your language to be implemented as pure BNF,
without rule ranking.
This will often be the best choice.
