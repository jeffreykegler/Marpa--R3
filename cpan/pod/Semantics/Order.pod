...
[ TODO: This needs to be rewritten to incorporate the changes in
Marpa::R3::Semantics::Rank.
]
...
Informally, a B<choicepoint> is a location where
a parsing decision must be made.
A decision may have a single choice,
in which case the B<decision> is called trivial.

For the parse choices to be interchangeable,
they all must share a single rule.
Because of this the rule of the choicepoint itself
is not the source of a choice's rank --
instead the rank of each choice's B<cause>
is used.
(Informally, a choice's cause might be thought
as the most recent child of the choicepoint,
when that choice is made.)
The parse choices of a choicepoint typically
have different causes.

The terms "choicepoint" and "cause"
are defined much more carefully
in L<a separate
document|Marpa::R3::Semantics::Rank>.
...
there will be B<choicepoints> in the parse bocage.
At the choicepoints, there will be two or more
B<parse choices> -- choices which
result in different parse trees.

When ranking, the logic traverses the parse bocage,
looking for choicepoints.
