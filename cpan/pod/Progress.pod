...
Between the rule tag and the expanded dotted rule,
are one or two fields
of location information.
If there are multiple origins for the dotted rule,
there will be two fields of location information.

In the example just above the rule has only
one origin, and the second field is "C<B1L1c1>".
...
"C<B1L1c1>" describes the L0 location of origin of the dotted rule in
the parse.
This dotted rule has only one origin, and it is at line 1, column 1
of block 1.
...
Here it is from the C<productions_show()> listing:
...
Next, we ask ourselves, what is the earliest place R18
should be appearing?
The answer is that
there should be a prediction of R18 at location 0.
...
No R18 predicted at G1 location 0.
Next we look through the the entire progress report, at all G1 locations,
to see if R18 is predicted anywhere.
No R18.  Not anywhere.

The LHS of R18 is C<E<lt>numeric expressionE<gt>>.
