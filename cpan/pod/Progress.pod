...
a pair, a production and a dot location are called a B<dotted rule>.
...
If there are multiple origins for the dotted rule,
...
Marpa allows you to move around arbitrarily in its input.
You can back up, overlap previous input, or anything else you choose.

But, by default,
your applications moves forward continuously in the input string.
Unless your app is specifically programmed to use Marpa's
ability to move around arbitrarily in the input,
you can ignore the considerations in this section.

In terms of Earley sets (G1 locations) there is no problem
if an application backtracks in the physcial input.
G1 locations do not reflect the physical input.
They always run in a monotonic sequence, starting with 0.
G1 locations never run backwards, they are never visited twice,
and they leave no gaps.

L0 location ranges are more problematic.
Specifically, when a G1 range is converted to an L0 range,
Marpa will report the L0 locations for the first and last
Earleys sets, ignoring what happens in between.
...
Intuitively,
this means that the range will be interpreting as the
start and end points in the input.
It is quite possible that, in between the start and end L0
locations, Marpa will have traversed L0 locations before the
start L0 location and after the end L0 location.
The L0 range, as reported, will give no indication that this has
happened.
...
"C<F7 x12 @0...38-41 B1L1c1-L2c40>",
that tells us
that the dotted rule is production 7, which has 12 instances.
All 12 instances have their dot location at G1 location 41,
but their origins are in the range from G1 location 0
to G1 location 38.

The last field in
"C<F7 x12 @0...38-41 B1L1c1-L2c40>" is an input string range.
...
of this dotted rule, both with a dot location of 41.
...
at which the dotted rule instance reported by
...
This is the G1 portion of the C<productions_show()> output at verbosity level 3.
