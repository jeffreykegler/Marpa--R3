...
indicating the number of instances of that dotted rule in
its Earley set -- in this case, that there are, respectively,
2, 12 and 20 instances of the reported dotted rule.

Every dotted rule instance is in the same Earley set,
that is, has the same G1 location.
Within an Earley set, an instance of the same dotted rule differs
for another instance of the same dotted rule
only in its origin.

The origin of a dotted rule instance is the location where it begins.
Within an Earley set
the instances of a single dotted rule may have many different origins --
hundreds or even more.

Each instance of a dotted rule
