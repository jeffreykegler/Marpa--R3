...
=item 4)
The last character in an input string range is the last one traversed, and

=item 5)
Every character in an input string range is traversed in L0 location order.
...
If you are not using the default behavior, you can only rely on
1) and 2) being true.
