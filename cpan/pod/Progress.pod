...
But, by default,
your application moves forward continuously in the input string.
Unless your app is specifically programmed to use Marpa's
ability to move around arbitrarily in the input,
you can ignore the considerations in the rest of this section.

In terms of Earley sets (G1 locations) there is no problem
if an application backtracks in the physical input.
...
The start of an L0 range is the first location
traversed.
The end of an L0 range is the last location
traversed.

This means that an L0 range is all about the end points.
In reporting an L0 range,
Marpa ignores whatever happens in between the first and last
L0 locations.
