...
Currently only one named argument is allowed, the C<recognizer> argument, and
that argument is required.
The value of the C<recognizer> argument must be a SLIF recognizer object.
...
Returns the B<G1 glade span>.
The first number will be the start of the span, as
an offset in the input stream.
The second number will be its length.
Both will be in G1 location terms.
The glade span remains constant within a glade.
The C<g1_span()> method accepts no arguments.

The "end" G1 location of the span, when defined, may be calculated as
