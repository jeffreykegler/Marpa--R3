...
Marpa::R3::DSL - The Marpa::R3 domain specific language
...
The Marpa::R3 DSL is
an extension of BNF.
The Marpa::R3 DSL is used to specify other
DSL's, and is therefore a "meta-DSL".

=head1 Lexical conventions

The Marpa::R3 DSL source string consists of a series of rules,
...
When a Marpa::R3 DSL symbol needs to be
...
Special characters are allowed in event names.
For example, Marpa::R3 DSL authors sometimes write
lexeme events using quote (C<'"a"'>),
predictions starting with a caret (C<'^a'>),
completions ending with a dollar sigh (C<'a$'>)
and nulling events ending with empty brackets
(C<'a[]'>)
or starting with a bang
(C<'!a'>).
These are conventions,
and 
entirely up to the Marpa::R3 DSL author.
The Marpa::R3 DSL allows any event to have any legal
event name.
...
The Marpa::R3 DSL consists of a series of statements.
...
=head2 Rules, alternatives and productions

A B<production> is a LHS associated with a RHS alternative.
Semantics are associated with productions and tracing
and L<the progress
mechanism|Marpa::R3::Progress> track parses by production.
You can determine what the productions are for a grammar
using the L<productions_show() grammar
method|Marpa::R3::Scanless::G/"productions_show()">.
...
L<C<event>|/"event">
adverbs are allowed.
The value of the C<event> adverb
must be an event initializer.
The format of an event initializer is
described
L<above|/"Event initializer">.
If present, it defines a discard event,
as described in
L<the document on parse
events|Marpa::R3::Event/"Discard events">.
...
L<C<lexeme_read_block()>|Marpa::R3::Scanless::R/"lexeme_read_block()">,
L<C<lexeme_read_literal()>|Marpa::R3::Scanless::R/"lexeme_read_literal()">,
L<C<lexeme_read_string()>|Marpa::R3::Scanless::R/"lexeme_read_string()">
and
...
with the current location at the event location.
Once triggered,
named events may be queried using
L<the Scanless recognizer's events()
method|Marpa::R3::Scanless::R/"events()">.
For details, see
L<the document on parse
events|Marpa::R3::Event>.
...
Marpa parses ambiguous grammars and the design of the Marpa::R3 DSL exploits this.
...
It will always be possible to disambiguate a Marpa::R3 DSL script,
...
There are many other ways to disambiguate Marpa::R3 DSL statements.
...
The C<eager> adverb applies only to lexemes and
is only allowed in
L<C<:lexeme> pseudo-rules|/"Lexeme pseudo-rule">
and
L<discard pseudo-rules|/"Discard pseudo-rule">.
Currently, "eagerness"
is always treated
...
The value of the C<eager> adverb is a boolean,
in the form of a binary digit (C<0> or C<1>).
The default is C<0>, which means "greedy"
matching for the lexeme or token.
A value of C<1> specifies "eager" matching.

In these Marpa documents,
it is assumed
that "greedy" matching,
or LATM (Longest Acceptable Token Matching),
is in effect.
This means that a Marpa token or lexeme will match
the longest token or lexeme that is acceptable to
the grammar.

If "eager" matching is in effect,
and any "eager" match is available,
then Marpa
will match the B<shortest eager> token or lexeme that is
acceptable to the grammar.
Intuitively, "eagerness" may be thought of as
overriding "greediness", even for greedy tokens.

Whatever the settings, all tokens matched by Marpa
will always be of the same length.
Several tokens may match at a single location,
and they may be all greedy,
all eager, or any combination of both.

If an "eager" token matches, all "greedy" tokens
of the same length will also match.
In this case, these "greedy" token matches may not be
the longest acceptable token of their kind
-- intuitively, "eagerness" overrides "greediness".
If this behavior is considered undesirable,
it can be changed
using the
L<priority|/"priority"> adverb.

Note that it is possible that
a matching "eager" token
may be longer than any of
the acceptable "greedy"
tokens.
In this case, the "eager" token will match
and the "greedy" tokens will be ignored.
Again, intuitively,
"eagerness" overrides "greediness".
...
The two adverbs must occur together:
It is a fatal error to specify the C<event> adverb
if the L<C<pause>|/"pause"> adverb is not also specified.
And
It is a fatal error to specify the C<pause> adverb
if the L<C<event>|/"event"> adverb is not also specified.
...
the grammar's C<ranking_method> named argument
...
is only allowed in
L<a C<:lexeme> pseudo-rule|/"Lexeme pseudo-rule">.
The C<pause> adverb declares a parse event.
The L<event adverb|/"event"> names the parse event
declared by
the C<pause> adverb.
It is a fatal error to specify the
C<pause> adverb if the C<event> adverb is
not also specified.
