...
=head1 Reading Tokens

Both internal and external scanning read tokens.
In internal scanning tokenization is done as described by
the L0 grammar.
...
Marpa::R3's
L<valuer objects|Marpa::R3::Scanless::V>
can be used.
...
C<lexeme_read_block()> is the most basic method for external scanning.
...
The C<lexeme_read_string()> differs from C<lexeme_read_block()>
in its arguments,
