...
Moves the current physical position to
block offset C<offset> in the block with index
C<block_ix>.
The end of read is set to C<length>.

If C<block_ix> is omitted, it defaults to the current
block.
If the C<length> is omitted it defaults to the
length of the remainder of the block
