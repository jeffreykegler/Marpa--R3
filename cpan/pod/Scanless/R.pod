...
Naively, you would expect position in the input
to too simple to be worth mentioning in a document.
Input is a string.  Scanning starts at the first
position and continues to the end of the string.
And Marpa::R3 can be used in this way.

But, with surprising frequency,
current practical applications
do not fit into this model.
Many practical languages want one or more of the following
...
=item *
"here-docs", which in one form,
is essentially the extension of "include file"
...
Marpa's underlying input model is very flexible,
and allows all of these things and more.

Marpa::R3 allows multiple input strings.
In these documents, these input strings are
called C<blocks>.
All blocks are Unicode,
and numbering in characters (which are the same
as Unicode codepoints) is 1-based.
...
While there is a one-to-one function from lexemes
to G1 positions, the opposite is not quite the case.
G1 position 0 is a G1 position before the reading
of the first lexeme.
This means that G1 position is 0-based.

G1 position is useful when dealing with the semantics.
The semantics is defined in terms of lexemes,
...
characters in whatever block
is considered "current" in context,
and end of universe is the last character of the ordered set.
...
The offset -I<N> is the same as the
offset I<E> - I<N> + 1,
where I<E> is the positive
...
[I<S ... E>].
A range is the sting of locations
...
When a block span is part of a set of arguments,
one or more elements of it may be undefined.
In this case, block ID defaults to the current block,
offset defaults to the current offset of the specified block
and length is set according to the eoread of the
specified block.
More formally:
...
=item *
When all elements of the triple are defined,
the block ID, offset and length are as explicitly
specified,
and they are treated as a block span as
described above.

=item *
If the C<block-id> is undefined,
it specifies the current block.
It is a fatal error if there is no current block.

=item *
If C<offset> is undefined,
it specified the current offset of the specified block.
It is a fatal error if there is no specified block.

=item *
If C<length> is undefined,
the length specified is C<eoread> - C<offset> + 1,
where C<eoread> is the eoread of the specified block.
It is a fatal error if there is no specified block.
...
using the
L<C<lexeme_read_block()>|/"lexeme_read_block()">
method call.
