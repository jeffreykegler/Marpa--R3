...
=head1 Position in Marpa

By default. position in Marpa
is the obvious -- there is one input string.
This input string is read one character at a time, from
beginning, which is character 1,
to the end, whose character number (aka B<offset>)
is the same as the length of the string.
...
Many practical languages have an "include file" facility,
which require a reader to be able,
at any point, to switch to a completely different input
string.
Many also use pre-processing which requires the reader
to skip forward over its input strings.
Languages which allow
here-docs require applications to move backward and
forward in the input.

Marpa's underlying input model is very flexible,
and allows all of these things and more.
Marpa allows multiple input strings (aka C<blocks>),
and it allows the app to switch between them
and to change position in them.

=head1 Block position

In its full generality,
Marpa has two idea of position.
The first and more frequently used is B<block position>.
The other idea of position is B<G1 position>,
...
Block position is a duple of B<block id> and
B<block offset>.
When the context makes the meaning clear,
"block offset" is often simply called "offset".
Block ids and block offsets are both 1-based.

=head1 G1 position

Above I described block position.
This is what the user is usually thinking of when
he says "position".
Marpa also tracks position in terms of lexemes.
A Marpa B<lexeme>
is similar to what is meant by "token"
elsewhere.
The semantics of a Marpa grammar in defined
in terms of lexemes, so that G1 position is
important when dealing with semantics.

Technically, G1 position is position in terms
of the G1 grammar's Earley sets.
Each Earley set except the first represents
the reading of a lexeme.
Earley set 0 (which is at G1 position 0)
is before the reading of any input.
Earley set 1 (which is at G1 position 1)
is after the reading of the first lexeme.
For every I<N>, Earley set I<N> is
at G1 position I<N>, and vice versa.

=head1 Negative offsets and positions

Negative offsets and positions are offsets
relative to an "end of universe",
where the universe varies by context.
For blocks, the universe is whatever block
is considered "current" in context,
and end of universe is the end of that block.
For G1 locations, the universe is the set of
G1 locations, and the end of universe is
the last valid G1 location.

The offset -I<N> is the same as the
offset I<E> - I<N> + 1,
where I<E> is the last element in the universe.
