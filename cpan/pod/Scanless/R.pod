...
=item *
"Include" files, as just described,
but also nest them.
...
An input text block is set to be the
current input text block
when one of the reading methods is called.
The L<C<read()>|/"read()">
...
or the app must moves the block offset of the current block.

As mentioned, every block, whether current or not,
maintains a current offset.
Every block also maintains
...
an app to tell the recognizer to use
