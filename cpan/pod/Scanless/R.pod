...
C<read()>
creates a new block whose text is set from C<$p_string>
and whose current offset and eoread is set as stated above.
It sets the current block to the newly created block.
C<read()> then
It starts internal scanning of the newly created block,
which is now the current block.
...
They are useful for both internal and external
scanning.
...
C<lexeme_alternative_literal()> differs from
C<lexeme_alternative()> only in how it sets the value of the token.
