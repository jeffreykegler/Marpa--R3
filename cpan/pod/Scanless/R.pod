...
But many practical applications
...
"here-docs",
which in one form,
are essentially the extension of "include file"
logic to in-line "files"

=item *
"if/else" logic, which requires the app to skip
over sections of the input

=item *
externally scanned symbols, which do not refer
to any text in the main input string.

=back

The above features require the app to switch
among multiple input strings,
some concocted on the fly,
as well as the ability to jump forward
and backward within those input strings.
Marpa::R3's input model
allows all of these things and more.
...
By default, the next character read will be
the character at the current block offset
of the current block.

The highest valid block offset is the
physical length of the block in characters.
When the block offset is equal to the physical
length of the block,
it indicates that the block has been read all
the way to the end,
and that there are no more characters to be read.
(But an app can continue reading characters,
if it switches blocks,
or if it moves the block offset of the current block.)
When context makes it clear,
the block offset is offered called
simply the C<offset>.

Once a recognizer starts reading,
one of its blocks is always
...
and a B<current end of read>.
The B<current end of read> is often
referred to as the block's B<end of read>
or B<eoread>.
When a block is initialized, 
its current offset is 1 and its eoread
is the last character of the block.

Reading of a block past its eoread is not allowed.
In effect,
eoread allows an app to tell the recognizer to use
...
In Marpa::R3, we often use
negative offsets and positions.
These are offsets
relative to an "end of universe",
where the universe varies by context.

For blocks, the universe is the ordered set of
characters in the input text block
under discussion,
and end of universe is the last character of this ordered set --
its B<physical end of block>.
Eoread does not affect end-of-universe.
A block's eoread may be different from its physical end-of-block,
but a block's end-of-universe is always its 
physical end-of-block.

For G1 locations, the universe is the ordered set of
G1 locations, and the end of universe is
...
Since block offsets are 1-based,
the last character in block 7 will be at offset 42,
and offset 42 will be the "end of universe".
Therefore,
...
is the same as block position C<7, 42>.
...
is the same as block position C<7, 1>.
...
consider again an input text block of length 100.

=over

=item *

The entire block is the range C<[1 ... -1]>
and the span C<[1, -1]>.

=item *

The first 42 characters of the input stream are the range C<[1 ... 42]>
and the span C<[1, 42]>.
...
In this case, C<block_id> ID defaults to the current block,
...
it allows the app to specify both a block span and an explicit value
for the semantics.
...
=head1 Direct block manipulation
...
Direct block manipulations
can be useful for both internal and external scanning.
...
It takes three arguments, the first of which is required.
Call them, in order, C<$p_string>, C<$start>, C<$length>.

C<$p_string> is required
and must be a pointer to a string.
A new input text block is created from C<$p_string>.
The current block is set to this new block.
C<read()> then parses this new current block according to the grammar.

The C<$start> argument is optional.
Parsing starts in the new block at offset C<$start>.
If C<$start> is undefined, it defaults to 1 -- the first
character of the new block.
In other words, the default is to read from the beginning
of the
new physical block.
If start is negative,
it is interpretated as
L<as defined above|/"Negative offsets and positions">.

The C<$length> argument is optional.
If defined, eoread is set to C<$start> + C<$length> - 1.
If not defined, eoread is set to the last physical character
of the new, current block.
In other words, the default is to read to the end of the
new physical block.
C<$length> may not be negative.
If C<$start> and $<length> produce an eoread which is outside
the new current block, it is a fatal error.

If C<$length> is zero, C<read()> set a new current block,
and then stops immediately.
This can be useful when an application wants to take control
before any internal scanning is done.
An application may, for example, use C<read()>
to set up a buffer but then immediate switch to external
scanning.
...
C<read()> returns success if it parses to eoread,
or if it triggers a parse event.
If a parse event occurs during the C<read()> method,
the current location of the current block
is set to the event location.
Parse events are described in detail in
L<a separate document|Marpa::R3::Event>.
If no parse event triggers,
and the parse reaches eoread without a
failure,
the current location is set one past eoread.

On success, C<read()> returns the current
offset of the new, current block.
...
valuer object|Marpa::R3::Scanless::V>.
...
The span is used, among other things,
...
TODO This method description is wrong
-- it describes R2's lexeme_read().
It needs to be totally rewritten.

The C<lexeme_read_block()> method reads a single, unambiguous, lexeme.
...
The lexeme value plays an important role in the Marpa::R3's semantics.
More details on the Marpa::R3's semantics are in
...
of the grammar|Marpa::R3::Scanless::G/"rule()">.
