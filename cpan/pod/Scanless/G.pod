...
will usually prefer
L<production_ids_gen()|
/"production_ids_gen()">.

If you do write your own loop,
remember that the lowest possible production ID 
is 1,
and that there may be gaps in the valid
ids -- it is up to the app to check for
invalid production IDs.
...
If you do write your own loop,
remember that the lowest possible symbol ID 
is 1,
and that there may be gaps in the valid
ids -- it is up to the app to check for
invalid symbol IDs.
...
name: SLG production_dotted_show() synopsis
    ...
    $production_dotted_results .= $grammar->production_dotted_show($production_id);
...
Return on error:
A Perl C<undef> if the production does not exist.
...
=for Marpa::R3::Display
name: SLG production_length() example
...
suitable for looping through them.
More specifically,
every call of C<symbol_ids_gen()> returns
either a valid symbol ID or a Perl C<undef>.
An iterator never returns the same symbol ID twice.
...
name: SLG symbol_show() synopsis
    ...
    my $symbol_description = $grammar->symbol_show($prid);
