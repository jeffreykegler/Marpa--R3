...
catches up to the furthest earleme,
by calling
the L<C<lexeme_complete>|/"lexeme_complete()"> method until
the furthest earleme is equal to the current earleme.

Events are another circumstance which may cause an application
to wish to "catch up" in the pending lexeme queue.
Lexemes in the pending lexeme queue do not cause events
until they are added to the parse
by the L<C<lexeme_complete>|/"lexeme_complete()"> method.
