...
Every lexeme may have three things associated with it:
...
The literal equivalent of a lexeme is not directly visible
to the semantics,
...
If more than one lexeme is accepted at a G1 location --
...
An external scanning methods succeeds and reads a lexeme
if no pre-lexeme event occurs.
In the case of a successful read,
the current block is set to C<$block_id>;
The current offset is set to C<eolexeme>;
and the current eoread of the current block will
not be changed.
...
and the G1 location of the lexeme is considered to be
...
If an external scanning completion method rejects a lexeme,
...
C<$symbol_name>
is the symbol name of a lexeme
to be read.
C<$string>
