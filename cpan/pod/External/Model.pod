...
in always 0;
and the current, closest and furthest earleme
...
the value of the closest earleme before and after the second and subsequent
...
Where the current earleme is I<c>,
the value of the furthest earleme before and after the second and subsequent
successful calls of C<lexeme_alternative()>
will be
I<c>+1.
...
the closest and furthest earlemes are equal to
each other;
and are equal to the current earleme.
...
introduced.
Here are some facts about the earleme variables that will always be the case,
